...
import MonthlyRefundsTable from './Tables/MonthlyRefundsTable';
...
function ManageRefunds(props) {
    document.title="Manage Refunds";
    ...
    const [urlc,setUrlC]=useState(par)
    const [id,setID]=useState((par.id)?par.id:'')
    const [month,setMonth]=useState((par.month)?par.month:'all')
    ...
    const [showcheckhouse,setShowCheckHouse]=useState(false)
    const [showchecktenant,setShowCheckTenant]=useState(false)
    ...
    useEffect(()=>{
        ...
        const getPrevMonths = async (e) => { 
            const arr = [];
                ...
                arr.push({value: 'all', label: 'All Months' });
            const arr1 = [];
                arr1.push({value: '', label: 'Select Property' });
                arr1.push({value: 'All', label: 'All' });
            const arr2 = [];    
            let url=`/v2/update/refunds/load/${id}/${month}`;
            if(id===''){
                if(month===''){
                    url=`/v2/update/refunds/load/all`;
                }
                else{
                    url=`/v2/update/refunds/load/${month}`;
                }
            }
            else{
                setLoadingMonths(false)
                return false;
                // url=`/v2/update/refunds/load/${id}/${month}`;
            }
            await axios.get(url)
            .then(response=>{
                if (doneloading) {
                    if(response.data.status=== 200){
                        ...
                        if(resthisproperty==''){
                            options={value: "All", label: 'All Property Refunds' , data: resthisproperty}
                        }
    ...
    useEffect(()=>{
        ...
        if(id!==''){
            // acceptedFiles[0]='';
            let thisurl=`/properties/mgr/refunds/${id}/${month}`;
            navigate(thisurl)
        }
    ...
    const closePreviewUpload= (e)=>{
        e.preventDefault();
        setLoadingMonths(true)
        axios.get(`/v2/update/refunds/load/${id}/${month}`)
        .then(response=>{
            if(response.data.status=== 200){
                setWaterbillData(response.data.waterbilldata);
                setPreview(response.data.preview);
...
// waterbillvaluesupdate
  ...
  return (
    ...
    <div className="wrapper">
        ...
        :
        <>
        <DashNavBar setClosed={setClosed} closed={closed} active='refunds'/>
        <DashSideNavBar setClosed={setClosed} closed={closed} active='refunds'/>
                    ...
                    <div className="container">
                            ...
                            <div className="col-12 m-0 p-0 ">
                                <ol className="breadcrumb float-sm-right text-xs m-0 p-1">
                                    ...
                                    {/* {!loadingmonths && waterbillpropertyid.value!=='All'?
                                        <>
                                            <li className="breadcrumb-item" ><Link to={'/properties/mgr/refunds/All/'+waterbillmonth.value} >All</Link> </li>
                        ...
                        <div className="col-lg-9 m-0 p-0">
                            <div className="row m-0 p-0">
                                        ...
                                        <div className="card-header bg-info text-white elevation-2 m-0 p-0">
                                            <p className='text-center p-1 m-0'>
                                                ...
                                                <span>
                                                    ...
                                                    {!loadingwater &&
                                                        <>
                                                            {waterbillpropertyid.label!==undefined && "Refund Status for::"+ waterbillpropertyid.label+" "+waterbillmonth.label} 
                                                            {preview && 
                                                    ...
                                                    <table border="1" className="table table-hover table-bordered text-xs" id="example1">
                                                        {waterbilldata  && waterbilldata &&
                                                            ...
                                                            <tr  style={{"color":"white","backgroundColor":"#77B5ED"}}>
                                                                ...
                                                                <th className='elevation-2 m-0 p-1'>Housename
                                                                    <button title='Show Current Tenant' className='btn text-white float-right m-0 p-0 pl-1 pr-1' onClick={()=>{setShowCheckHouse(!showcheckhouse)}}>{showcheckhouse? <i className='fa fa-eye-slash  '></i>: <i className='fa fa-eye '></i>}</button>
                                                                </th>
                                                                <th className='elevation-2 m-0 p-1'>Tenant
                                                                <button title='Show Current House' className='btn text-white float-right m-0 p-0 pl-1 pr-1' onClick={()=>{setShowCheckTenant(!showchecktenant)}}>{showchecktenant? <i className='fa fa-eye-slash  '></i>: <i className='fa fa-eye '></i>}</button>
                                                                </th>
                                                                <th className='elevation-2 m-0 p-1'>Deposit</th>
                                                                <th className='elevation-2 m-0 p-1'>Balance</th>   
                                                                <th colSpan={3} className='elevation-2 m-0 p-1'>
                                                                    Charges
                                                                    <br/>
                                                                    <small>Damages|Transaction|Others</small>
                                                                </th>      
                                                                {/* <th className='elevation-2 m-0 p-1'>Damages</th>
                                                                <th className='elevation-2 m-0 p-1'>Transaction</th> */}
                                                                <th className='elevation-2 m-0 p-1'>Refund</th>
                                                                <th className='elevation-2 m-0 p-1'>Staying<br/> Since</th>
                                                                <th className='elevation-2 m-0 p-1'>Left<br/> ON</th>
                                                                ...
                                                                {(search.value==='')?
                                                                    <>
                                                                        {waterbilldata  && waterbilldata.map((waterbill,key) => (
                                                                            <MonthlyRefundsTable waterbill={waterbill} key={key} no={key} preview={preview} handleShow={handleShow} uploadwaterbill={uploadwaterbill} allchecked={allchecked} handleChange={handleChange} showcheckhouse={showcheckhouse} showchecktenant={showchecktenant} />
                                                                        ))
                                                                        }
                                                                    </>
                                                                :
                                ...
                                {show && 
                                    <AddRefundBills show={show} handleClose={handleClose} currentwaterbill={currentwaterbill} loadRentGarbage={loadRentGarbage}/>
                                }
