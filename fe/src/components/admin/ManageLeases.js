...
function ManageLeases(props) {
    document.title="Manage Leases";
    ...
    useEffect(()=>{
        ...
        const getPrevMonths = async (e) => { 
            ...
            const arr2 = [];    
            let url=`/v2/update/leases/load/${id}/${month}`;
            if(id===''){
                if(month===''){
                    url=`/v2/update/leases/load/all`;
                }
                else{
                    url=`/v2/update/leases/load/${month}`;
                }
            }
            else{
                setLoadingMonths(false)
                return false;
                // url=`/v2/update/leases/load/${id}/${month}`;
            }
            await axios.get(url)
            .then(response=>{
                if (doneloading) {
                    if(response.data.status=== 200){
                        ...
                        if(resthisproperty==''){
                            options={value: "All", label: 'All Property Leases' , data: resthisproperty}
                        }
    ...
    useEffect(()=>{
        ...
        if(id!==''){
            // acceptedFiles[0]='';
            let thisurl=`/properties/mgr/leases/${id}/${month}`;
            navigate(thisurl)
        }
    ...
    const closePreviewUpload= (e)=>{
        e.preventDefault();
        setLoadingMonths(true)
        axios.get(`/v2/update/leases/load/${id}/${month}`)
        .then(response=>{
            if(response.data.status=== 200){
                setWaterbillData(response.data.waterbilldata);
                setPreview(response.data.preview);
...
// waterbillvaluesupdate
  ...
  return (
    ...
    <div className="wrapper">
        ...
        :
        <>
        <DashNavBar setClosed={setClosed} closed={closed} active='leases'/>
        <DashSideNavBar setClosed={setClosed} closed={closed} active='leases'/>
                            ...
                            <div className="col-12 m-0 p-0 ">
                                <ol className="breadcrumb float-sm-right text-xs m-0 p-1">
                                    ...
                                    {/* {!loadingmonths && waterbillpropertyid.value!=='All'?
                                        <>
                                            <li className="breadcrumb-item" ><Link to={'/properties/mgr/leases/All/'+waterbillmonth.value} >All</Link> </li>
                                    ...
                                    <li className="breadcrumb-item active"> Leases </li>
                                </ol>
                            </div>
                                        ...
                                        <div className="card-header bg-info text-white elevation-2 m-0 p-0">
                                            <p className='text-center p-1 m-0'>
                                                ...
                                                <span>
                                                    ...
                                                    {!loadingwater &&
                                                        <>
                                                            {waterbillpropertyid.label!==undefined && "Leases Status for::"+ waterbillpropertyid.label+" "+waterbillmonth.label} 
                                                            {preview && 
                                                    ...
                                                    <table border="1" className="table table-hover table-bordered text-xs" id="example1">
                                                        {waterbilldata  && waterbilldata &&
                                                            ...
                                                            <tr  style={{"color":"white","backgroundColor":"#77B5ED"}}>
                                                                ...
                                                                <th className='elevation-2 m-0 p-1'>Lease<br/> Since</th>
                                                                ...
                                                                {(search.value==='')?
                                                                    <>
                                                                        {waterbilldata  && waterbilldata.map((waterbill,key) => (
                                                                            <MonthlyLeasesTable waterbill={waterbill} key={key} no={key} preview={preview} handleShow={handleShow} uploadwaterbill={uploadwaterbill} allchecked={allchecked} handleChange={handleChange} showcheckhouse={showcheckhouse} showchecktenant={showchecktenant} />
                                                                        ))
                                                                        }
                                                                    </>
                                                                :
