...
function ManagePropertyAssignHseTenant(props) {
    ...
    useEffect(()=>{
        ...
        const getProperties = async (e) => { 
            ...
            if(tenant===''){
                ...
                url=`/v2/properties/mgr/tenants/assign/${house===''?"None":house}/${tenant}`;
            }
    ...
    const loadAssigning =(thihouse,thistenant) =>{
        ...
        const getProperties = async (e) => { 
            ...
            if(thistenant==='' || thihouse===''){
                ...
                url=`/v2/properties/mgr/tenants/assign/${thihouse===''?"None":thihouse}/${thistenant}`;
            }
    ...
    const assignTenant= (house)=>{
        ...
        .then((willcontinue) =>{
            if(willcontinue){
                ...
                axios.post('/v2/assign/house/save',form)
                .then(response=>{
                    if(response.data.status=== 200){
    ...
    function handleTenantChange(val) {
        ...
        if(tenantid!==''){
            ...
            let thisurl=`/properties/mgr/tenants/${tenantid}/assign/${house}`;
            navigate(thisurl)
    ...
    function handleHouseChange(val) {
        ...
        if(houseid!=='' && tenant!==''){
            setHouseID(houseid)
            setTenantID(tenant)
            let thisurl=`/properties/mgr/tenants/${tenant}/assign/${houseid}`;
            navigate(thisurl)
                        ...
                        <div className="p-2">
                            ...
                            <div className="col-12 m-0 p-0 ">
                                <ol className="breadcrumb float-sm-right text-xs m-0 p-1">
                                    ...
                                    <li className="breadcrumb-item active">Assigning to: { !loadingmonths && housedata.Housename && housedata.Housename} </li>
                                </ol>
                            </div>
                                                    ...
                                                    <div className="col-12 m-0 p-1 mt-1">
                                                        { !loadingmonths &&
                                                            <p className='text-info mx-auto text-center'>
                                                                {tenantdata.Status==="New" || tenantdata.Status==="Vacated"?'Assigning':'Assigned'} Tenant 
                                                                <span className='text-success'> { !loadingmonths && tenantdata.tenantname }
                                                    ...
                                                    <div className="col-12 col-lg-6 text-left m-0 p-1 mt-1 mb-2">
                                                        <div className="card border-ok m-2 p-1" >
                                                            <div className="card-header bg-success border-info text-muted text-center m-0 mb-2 p-2 pt-1 pb-2">
                                                                ...
                                                                <span className='m-0 p-1 text-dark mx-auto'>{housedata.Housename}</span>
                                                            ...
                                                            <div className="card-body text-center text-muted text-sm m-0 p-0 pt-1">
                                                                ...
                                                                <div className='col-12 m-0 p-1 elevation-0 border-none'>
                                                                    ...
                                                                    {housedata.Rent >0 &&
                                                                        <label className='m-0 p-1 text-xs border-info assignlbl' title='House Rent'>Rent: 
                                                                            <span className='bold assigntxt'> {new Number(housedata.Rent).toFixed(2)}</span> {formdata.HouseRent? <i className='fa fa-check text-lime'></i>:''}
                                                                        </label>
                                                                    }
                                                                    {housedata.Garbage >0 &&
                                                                    <label className='m-0 p-1 text-xs border-info assignlbl' title='Garbage'>Garbage: 
                                                                        <span className='bold assigntxt'> {new Number(housedata.Garbage).toFixed(2)}</span> {formdata.Garbage? <i className='fa fa-check text-lime'></i>:''}
                                                                    </label>
                                                                    }

                                                                    {housedata.Deposit >0 &&
                                                                    <label className='m-0 p-1 text-xs border-info assignlbl' title='House Deposit'>Deposit: 
                                                                        <span className='bold assigntxt'> {new Number(housedata.Deposit).toFixed(2)}</span> {formdata.HouseDeposit? <i className='fa fa-check text-lime'></i>:''}
                                                                    </label>
                                                                    }
                                                                    {housedata.Kplc >0 &&
                                                                    <label className='m-0 p-1 text-xs border-info assignlbl' title='KPLC Deposit'>KPLC D: 
                                                                        <span className='bold assigntxt'> {new Number(housedata.Kplc).toFixed(2)}</span> {formdata.KPLCDeposit? <i className='fa fa-check text-lime'></i>:''}
                                                                    </label>
                                                                    }

                                                                    {housedata.Water >0 &&
                                                                    <label className='m-0 p-1 text-xs border-info assignlbl' title='Water Deposit'>Water D: 
                                                                        <span className='bold assigntxt'> {new Number(housedata.Water).toFixed(2)}</span> {formdata.WaterDeposit? <i className='fa fa-check text-lime'></i>:''}
                                                                    </label>
                                                                    }
                                                                    {housedata.Lease >0 &&
                                                                    <label className='m-0 p-1 text-xs border-info assignlbl' title='Lease'>Lease: 
                                                                        <span className='bold assigntxt'> {new Number(housedata.Lease).toFixed(2)}</span> {formdata.Lease? <i className='fa fa-check text-lime'></i>:''}
                                                                    </label>
                                                                    }
                                                                ...
                                                                <div className="col-12 m-0 p-0">
                                                                    {housedata.Rent >0 &&
                                                                        <label className={`m-0 p-1 text-xs ${formdata.HouseRent?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`} title='House Rent Charges'>
                                                                    ...
                                                                    {housedata.Garbage >0 &&
                                                                        <label className={`m-0 p-1 text-xs ${formdata.Garbage?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`}  title='Garbage Charges' >
                                                                    ...
                                                                    {housedata.Deposit >0 &&
                                                                        <label className={`m-0 p-1 text-xs ${formdata.HouseDeposit?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`} title='House Deposit' >
                                                                    ...
                                                                    {housedata.Kplc >0 &&
                                                                        <label className={`m-0 p-1 text-xs ${formdata.KPLCDeposit?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`} title='KPLC Deposit'  >
                                                                    ...
                                                                    {housedata.Water >0 &&
                                                                        <label className={`m-0 p-1 text-xs ${formdata.WaterDeposit?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`} title='Water Deposit'  >
                                                                    ...
                                                                    {housedata.Lease >0 &&
                                                                        <label className={`m-0 p-1 text-xs ${formdata.Lease?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`}  >
                                                                    ...
                                                                    {housedata.Rent+housedata.Garbage >0 &&
                                                                        <label className={`m-0 p-1 text-xs ${formdata.MonthlyCharge?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`}  >
                                                                ...
                                                                <div className="col-12 m-0 p-0">
                                                                    {!formdata.CustomRefund &&
                                                                        <>
                                                                        {housedata.Garbage >0 &&
                                                                            <label className={`m-0 p-1 text-xs ${formdata.GarbageRefund?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`}  title='GarbageRefund Charges' >
                                                                        ...
                                                                        {housedata.Deposit >0 &&
                                                                            <label className={`m-0 p-1 text-xs ${formdata.HouseDepositRefund?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`} title='House Deposit' >
                                                                        ...
                                                                        {housedata.Kplc >0 &&
                                                                            <label className={`m-0 p-1 text-xs ${formdata.KPLCDepositRefund?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`} title='KPLC Deposit'  >
                                                                        ...
                                                                        {housedata.Water >0 &&
                                                                            <label className={`m-0 p-1 text-xs ${formdata.WaterDepositRefund?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`} title='Water Deposit'  >
                                                                        ...
                                                                        {housedata.Lease >0 &&
                                                                            <label className={`m-0 p-1 text-xs ${formdata.LeaseRefund?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`}  >
                                                                    ...
                                                                    <label className={`m-0 p-1 text-xs ${formdata.CustomRefund?'bg-success text-white':'bg-white text-black'} border-info m-1 assignlbl`}  >
                                                            ...
                                                            <div className="col-12 col-lg-6 text-left m-0 p-1 mt-3 mb-2">
                                                                <div className="border-info card p-1 elevation-0">
                                                                    ...
                                                                    <div className="form-group row m-0 p-1 pb-2">
                                                                        ...
                                                                        <div className="col-sm-8 col-12 m-0 p-0">
                                                                            {formdata.CustomRefund?
                                                                                <input id="Refund" type="text" className="form-control" name="Refund" value={formdata.Refund} onChange={handleInputChange} placeholder="0.00" required  autoFocus/>
                                                                                :
                                                                                <input id="Refund" type="text" className="form-control" name="Refund" value={(new Number(new Number(formdata.HouseRentRefund?housedata.Rent:0.00) + new Number(formdata.GarbageRefund?housedata.Garbage:0.00) + new Number(formdata.HouseDepositRefund?housedata.Deposit:0.00) + new Number(formdata.KPLCDepositRefund?housedata.Kplc:0.00) + new Number(formdata.WaterDepositRefund?housedata.Water:0.00) + new Number(formdata.LeaseRefund?housedata.Lease:0.00) )).toFixed(2)} onChange={handleInputChange} placeholder="0.00" required readOnly autoFocus/>
                                                                            }
                                                                ...
                                                                <div className="card-body text-center text-muted text-sm m-0 p-0 pt-1">
                                                                    <div className='row m-0 p-1 elevation-0 border-none'>
                                                                        <h4>You Can do the following meanwhile!!</h4>
                                                                            <p>
                                                                                <button className='bg-white m-0 mt-1 ml-1 pt-1 pl-2 pr-2 pb-1 border-info text-info'><Link to={'/properties/mgr/tenants/'+housedata.tenant+'/addhouse/'} className='text-info'><i className='fa fa-plus-circle'></i> Add House</Link></button>
...
export default ManagePropertyAssignHseTenant;