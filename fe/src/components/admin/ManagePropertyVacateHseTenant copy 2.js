...
function ManagePropertyVacateHseTenant({currentpropertyhousesel,showvacatehouse,handleCloseVacateHouse,handleShowAddHouse, handleCloseAddProperty,loadTenants}) {
   
    const navigate=useNavigate();
    const [plot,setPlotID]=useState((currentpropertyhousesel===undefined)?'':currentpropertyhousesel.pid)
    const [id,setID]=useState((currentpropertyhousesel===undefined)?'':currentpropertyhousesel.hid)
        ...
        <Modal size='xl' show={showvacatehouse} onHide={handleCloseVacateHouse} className='text-sm m-2 mt-4'>
            ...
            <Modal.Header className='justify-content-center bg-danger m-0 p-2' closeButton>
                <Modal.Title className='mx-auto text-white'> 
                    <h5>Vacate {housedata && housedata.tenantname} from House : {currentpropertyhousesel !==undefined && currentpropertyhousesel.Housename}</h5>
                </Modal.Title>
            </Modal.Header>
