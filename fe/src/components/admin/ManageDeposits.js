...
function ManageDeposits(props) {
    document.title="Manage Deposits";
    ...
    useEffect(()=>{
        ...
        const getPrevMonths = async (e) => { 
            ...
            const arr2 = [];    
            let url=`/v2/update/deposits/load/${id}/${month}`;
            if(id===''){
                if(month===''){
                    url=`/v2/update/deposits/load/all`;
                }
                else{
                    url=`/v2/update/deposits/load/${month}`;
                }
            }
            else{
                setLoadingMonths(false)
                return false;
                // url=`/v2/update/deposits/load/${id}/${month}`;
            }
            await axios.get(url)
            .then(response=>{
                if (doneloading) {
                    if(response.data.status=== 200){
                        ...
                        if(resthisproperty==''){
                            options={value: "All", label: 'All Property Deposits' , data: resthisproperty}
                        }
    ...
    useEffect(()=>{
        ...
        if(id!==''){
            // acceptedFiles[0]='';
            let thisurl=`/properties/mgr/deposits/${id}/${month}`;
            navigate(thisurl)
        }
    ...
    const closePreviewUpload= (e)=>{
        e.preventDefault();
        setLoadingMonths(true)
        axios.get(`/v2/update/deposits/load/${id}/${month}`)
        .then(response=>{
            if(response.data.status=== 200){
                setWaterbillData(response.data.waterbilldata);
                setPreview(response.data.preview);
...
// waterbillvaluesupdate
  ...
  return (
    ...
    <div className="wrapper">
        ...
        :
        <>
        <DashNavBar setClosed={setClosed} closed={closed} active='deposits'/>
        <DashSideNavBar setClosed={setClosed} closed={closed} active='deposits'/>
                            ...
                            <div className="col-12 m-0 p-0 ">
                                <ol className="breadcrumb float-sm-right text-xs m-0 p-1">
                                    ...
                                    {/* {!loadingmonths && waterbillpropertyid.value!=='All'?
                                        <>
                                            <li className="breadcrumb-item" ><Link to={'/properties/mgr/deposits/All/'+waterbillmonth.value} >All</Link> </li>
                                        ...
                                        <div className="card-header bg-info text-white elevation-2 m-0 p-0">
                                            <p className='text-center p-1 m-0'>
                                                ...
                                                <span>
                                                    ...
                                                    {!loadingwater &&
                                                        <>
                                                            {waterbillpropertyid.label!==undefined && "Deposits Status for::"+ waterbillpropertyid.label+" "+waterbillmonth.label} 
                                                            {preview && 
                                                                ...
                                                                {(search.value==='')?
                                                                    <>
                                                                        {waterbilldata  && waterbilldata.map((waterbill,key) => (
                                                                            <MonthlyDepositsTable waterbill={waterbill} key={key} no={key} preview={preview} handleShow={handleShow} uploadwaterbill={uploadwaterbill} allchecked={allchecked} handleChange={handleChange} showcheckhouse={showcheckhouse} showchecktenant={showchecktenant} />
                                                                        ))
                                                                        }
                                                                    </>
                                                                :
