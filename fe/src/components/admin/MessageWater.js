...
function MessageWater(props) {
    ...
    const {socket,socketMessage, setsocketMessage,socketarr,loggedname, setLoggedName, loggedtoken, setLoggedToken, loggedpermissions, setLoggedPermissions, loggedroles, setLoggedRoles, loggedrole,setLoggedRole,sitedata,setSiteData} =useContext(LoginContext);
    ...
    useEffect( () =>{
        socket.on('waterbill_changed', (msg) =>{
            ...
            // setsocketMessage((socketMessage) => [...socketMessage,msgs1]);
            // setShowToast(true);
        ...
        socket.on('message_sent', (msg) =>{
            // let msgs1="Waterbill Updated for :"+msg;
            // setsocketMessage("Message has been Sent");
            setsocketMessage((prevMessages) => [...prevMessages,msg]);
            setShowToast(true);
            loadMessages();
        })
                        ...
                        {/* <div aria-live='polite'
                            ...
                            className='m-0 p-0 bg-dark position-relative' >
                                <ToastContainer position='top-end' className='m-0 p-0' style={{zIndex:999,opacity:.8}}>
                                    <AddToast handleCloseToast={handleCloseToast} showtoast={showtoast} message={socketarr} />
                                    {console.log(socketMessage)}
                                    {socketMessage.map((message,key) =>( 
                                        <>
                                        {console.log(message)}
                                        <AddToast key={key} no={key} handleCloseToast={handleCloseToast} showtoast={showtoast} message={message} />
                                        </>
                                    )) 
                                    }
                                </ToastContainer>
