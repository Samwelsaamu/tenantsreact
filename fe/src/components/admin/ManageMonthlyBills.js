...
function ManageMonthlyBills(props) {
    document.title="Update Monthly Bills";
    ...
    useEffect(()=>{
        ...
        const getPrevMonths = async (e) => { 
            ...
            const arr2 = [];    
            let url=`/v2/update/monthlybills/load/${id}/${month}`;
            if(id===''){
                ...
                return false;
                // url=`/v2/update/monthlybills/load/${id}/${month}`;
            }
            await axios.get(url)
            .then(response=>{
                if (doneloading) {
    ...
    useEffect(()=>{
        if(id!==''){
            acceptedFiles[0]='';
            let thisurl=`/properties/update/monthlybills/${id}/${month}`;
            navigate(thisurl)
        }
    ...
    const closePreviewUpload= (e)=>{
        e.preventDefault();
        setLoadingMonths(true)
        axios.get(`/v2/update/monthlybills/load/${id}/${month}`)
        .then(response=>{
            if(response.data.status=== 200){
                setWaterbillData(response.data.waterbilldata);
                setPreview(response.data.preview);
...
// waterbillvaluesupdate
  ...
  return (
    ...
    <div className="wrapper">
        ...
        :
        <>
        <DashNavBar setClosed={setClosed} closed={closed} active='monthlybills'/>
        <DashSideNavBar setClosed={setClosed} closed={closed} active='monthlybills'/>
                        ...
                        <div className="row justify-content-center m-0 p-0">
                            <div className="col-12 m-0 p-0 ">
                                <ol className="breadcrumb float-sm-right text-xs m-0 p-1">
                                    ...
                                    <li className="breadcrumb-item active"> All Monthly Bills </li>
                                </ol>
                            </div>
...
export default ManageMonthlyBills;