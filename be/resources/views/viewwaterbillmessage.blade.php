...
@section('title','View Waterbill Messages | Wagitonga Agencies Limited')
@section('HeaderTitle')
<div class="col-sm-6">
    <h5 class="m-0">View Waterbill Messages to @if($thisproperty!="")
                          ...
                          @forelse($propertyinfo as $propertys)
                            @if($thisproperty!="" && $watermonth!="")
                              @if($thisproperty->id==$propertys->id)
                                <option value="/properties/view/messages/waterbill/{{ $propertys->id }}/{{$watermonth}}" selected>{{ $loop->index+1 }}. {{ $propertys->Plotname }} ({{ $propertys->Plotcode }})</option>
                              @else
                                <option value="/properties/view/messages/waterbill/{{ $propertys->id }}/{{$watermonth}}">{{ $loop->index+1 }}. {{ $propertys->Plotname }} ({{ $propertys->Plotcode }})</option>
                              @endif
                            @elseif($thisproperty!="" && $watermonth=="")
                              @if($thisproperty->id==$propertys->id)
                                <option value="/properties/view/messages/waterbill/{{ $propertys->id }}/{{App\Http\Controllers\TenantController::getAddWaterMonthsThis()}}" selected>{{ $loop->index+1 }}. {{ $propertys->Plotname }} ({{ $propertys->Plotcode }})</option>
                              @else
                                <option value="/properties/view/messages/waterbill/{{ $propertys->id }}/{{App\Http\Controllers\TenantController::getAddWaterMonthsThis()}}">{{ $loop->index+1 }}. {{ $propertys->Plotname }} ({{ $propertys->Plotcode }})</option>
                              @endif
                            @else
                              <option value="/properties/view/messages/waterbill/{{$propertys->id}}/{{App\Http\Controllers\TenantController::getAddWaterMonthsThis()}}">{{ $loop->index+1 }}. {{ $propertys->Plotname }} ({{ $propertys->Plotcode }})</option>
                            @endif
                    ...
                    <div class="col-sm-4">
                      <select class="form-control select2" name="allproperties" onchange="location=this.value;" style="width: 100%;">
                        <option value="">Choose Month</option>
                          @if($thisproperty!="" && $watermonth!="")
                            {{App\Http\Controllers\TenantController::getViewWaterMonths($thisproperty->id,$watermonth)}}
                          @elseif($thisproperty!="" && $watermonth=="")
                            {{App\Http\Controllers\TenantController::getViewWaterMonths($thisproperty->id,App\Http\Controllers\TenantController::getAddWaterMonthsThis())}}
                          @endif
                      </select>
                    </div>
                            ...
                            @forelse($housesinfo as $houses)
                              @if($thishouse!="")
                                @if($thishouse->id==$houses->id)
                                  <option value="/properties/view/messages/waterbill/{{ $houses->Plot }}/{{$watermonth}}/{{$houses->id}}" selected>{{ $loop->index+1 }}. {{ $houses->Housename }} ({{ $houses->Status }})-->{{App\Http\Controllers\TenantController::getRecordedStatus($houses->id,$watermonth)}}</option>
                                @else
                                  <option value="/properties/view/messages/waterbill/{{ $houses->Plot }}/{{$watermonth}}/{{$houses->id}}">{{ $loop->index+1 }}. {{ $houses->Housename }} ({{ $houses->Status }})-->{{App\Http\Controllers\TenantController::getRecordedStatus($houses->id,$watermonth)}}</option>
                                @endif
                              @else
                                <option value="/properties/view/messages/waterbill/{{ $houses->Plot }}/{{$watermonth}}/{{$houses->id}}">{{ $loop->index+1 }}. {{ $houses->Housename }} ({{ $houses->Status }})-->{{App\Http\Controllers\TenantController::getRecordedStatus($houses->id,$watermonth)}}</option>
                              @endif
                ...
                <div class="card-body" style="padding-top: 2px;">
                    ...
                    <div class="col-sm-12" style="padding: 2px;margin: 0px;">
                      ...
                      <div class="card-header bg-info" style="padding: 4px;">
                        ...
                        <span class="" style="font-size: 15px;">View Waterbill Messages / 
                                    @if($thisproperty!="")
                                ...
                                @forelse($waterbillmessage as $thiswatermessage)
                                  ...
                                  <div class="card-body bg-primary" style="padding: 5px;margin: 3px;">
                                    <span class="text-light"><b>{{ App\Http\Controllers\TenantController::TenantNames($thiswatermessage['tid']) }}, {{ App\Models\Property::getHouseName($thiswatermessage['hid']) }} {{ App\Http\Controllers\TenantController::TenantPhone($thiswatermessage['tid']) }} </b> 
                                      @if(App\Http\Controllers\TenantController::getSentDateMsg($thiswatermessage['hid'],$watermonth)!="")
                                        <i class="float-right">{{ App\Http\Controllers\TenantController::getSentDateMsg($thiswatermessage['hid'],$watermonth) }}</i>
                                      @else
