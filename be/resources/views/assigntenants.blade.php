...
@section('title','Assign Tenant | Wagitonga Agencies Limited')
@section('HeaderTitle')
<div class="col-sm-4">
            ...
            <div class="card" style="border: none;">
                <div class="card-header" style="padding-top: 10px;">
                    <h4 style="text-align: center;"><button class="btn btn-success btn-small" style="padding: 2px;font-size: 13px;"><i class="fa fa-plus"></i> <a href="/properties/newtenant" style="color: white;">Add New</a></button>
                        Assign Tenant {{ $thistenant->Fname }} {{ $thistenant->Oname }} to  @if($thishouse!="")
                ...
                <div class="card-body" style="padding-top: 2px;">
                         ...
                         <div class="text-center">
                          ...
                          <div class="modal-footer justify-content-between bg-primary" style="padding: 3px;">
                            ...
                            @if($thistenant->Status=="New")
                              <a href="/properties/Assign/Tenant/{{$thistenant->id}}" class="btn btn-success btn-sm " style="padding: 3px;font-size: 15px;margin-right: 3px;color: white;"><i style="color: white;" class="fa fa-plus"></i> Assign</a>
                            @elseif($thistenant->Status=="Vacated")
                              <a href="/properties/Agreement/Tenant/{{$thistenant->id}}" class="btn btn-success btn-sm " style="padding: 3px;font-size: 15px;margin-right: 3px;color: white;" title="Tenant Agreement"><span class="fa fa-thumbs-up fa-1x"></span></a>
                            @elseif($thistenant->Status=="Assigned" || $thistenant->Status=="Reassigned" || $thistenant->Status=="Transferred")
                              <a href="/properties/Agreement/Tenant/{{$thistenant->id}}" class="btn btn-success btn-sm " style="padding: 3px;font-size: 15px;margin-right: 3px;color: white;" title="Tenant Agreement"><span class="fa fa-thumbs-up fa-1x"></span></a>
                              <a href="/properties/Add/House/Tenant/{{$thistenant->id}}" class="btn btn-info btn-sm " style="padding: 3px;font-size: 15px;margin-right: 3px;color: white;" title="Add House"><span class="fa fa-plus fa-1x"></span></a>
                              <a href="/properties/Transfer/Internal/Tenant/{{$thistenant->id}}" class="btn btn-primary btn-sm " style="padding: 3px;font-size: 15px;margin-right: 3px;color: white;" title="Internal Transfer"><span class="fa fa-paper-plane fa-1x"></span></a>
                              <a href="/properties/Design/Tenant/{{$thistenant->id}}" class="btn btn-warning btn-sm " style="padding: 3px;font-size: 15px;margin-right: 3px;color: white;" title="Design Tenant"><span class="fa fa-times fa-1x"></span></a>
                              <a href="/properties/Vacate/Tenant/{{$thistenant->id}}" class="btn btn-danger btn-sm " style="padding: 3px;font-size: 15px;margin-right: 3px;color: white;" title="Vacate Tenant"><span class="fa fa-times-circle fa-1x"></span></a>
                    ...
                    <div class="card direct-chat direct-chat-primary col-lg-6 col-sm-6 col-xs-6" style="margin: 5px;padding: 2px;">
                      <div class="card-header" style="padding: 4px;">
                        <span class="" style="font-size: 12px;">Assign  <b>{{ $thistenant->Fname }} {{ $thistenant->Oname }}</b> Here to 
                          @if($thishouse!="")
                          <b>{{ $thishouse->Housename }}</b>
                          @endif
                        </span>
                          ...
                          <select class="form-control select2" name="alltenants" onchange="location=this.value;" style="width: 100%;">
                            ...
                            @if($thishouse!="")
                              @if($thishouse->id==$houses->id)
                                <option value="/properties/Assign/Tenant/{{$houses->id}}/{{$thistenant->id}}" selected>{{ $loop->index+1 }}. {{ $houses->Housename }} ({{ $houses->Status }}) </option>
                              @else
                                <option value="/properties/Assign/Tenant/{{$houses->id}}/{{$thistenant->id}}">{{ $loop->index+1 }}. {{ $houses->Housename }} ({{ $houses->Status }}) </option>
                              @endif
                            @else
                              <option value="/properties/Assign/Tenant/{{$houses->id}}/{{$thistenant->id}}">{{ $loop->index+1 }}. {{ $houses->Housename }} ({{ $houses->Status }}) </option>
                            @endif
                        ...
                        <div class="card-body row">
                            ...
                            <div class="col-sm-6">
                              @if($thistenant->Status=="Assigned" || $thistenant->Status=="Reassigned" || $thistenant->Status=="Transferred")
                                <h4 class="text-success"> Tenant is Already Assigned</h4>
                              @else
                              <form role="form" class="form-horizontal" method="POST" action="{{ route('agreement.store') }}">
                                <div class="row">
                                @csrf
                                <div class="col-sm-12">
