...
@section('title','Transfer Tenant Here | Wagitonga Agencies Limited')
@section('HeaderTitle')
<div class="col-sm-4">
    <h6 class="m-0">Transfer  @if($transferthistenant!="")
                           <b>{{ $transferthistenant->Fname }} {{ $transferthistenant->Oname }}</b>
                          @endif
                          to @if($thishouse!="")
                          {{ $thishouse->Housename }}
                          @endif</h6>
...
<ol class="breadcrumb float-sm-right">
  ...
  <li class="breadcrumb-item active">Transfer @if($transferthistenant!="")
                           <b>{{ $transferthistenant->Fname }} {{ $transferthistenant->Oname }} </b>
                          @endif
                           : @if($thishouse!="")
                          {{ $thishouse->Housename }}
            ...
            <div class="card" style="border: none;">
                <div class="card-header" style="padding-top: 10px;">
                    <h4 style="text-align: center;"><button class="btn btn-success btn-small" style="padding: 2px;font-size: 13px;"><i class="fa fa-plus"></i> <a href="/properties/newtenant" style="color: white;">Add New</a></button>
                        Transfer @if($transferthistenant!="")
                           <b>{{ $transferthistenant->Fname }} {{ $transferthistenant->Oname }}</b>
                          @endif
                          :  @if($thishouse!="")
                ...
                <div class="card-body" style="padding-top: 2px;">
                    ...
                    <div class="card direct-chat direct-chat-primary col-lg-6 col-sm-6 col-xs-6" style="margin: 5px;padding: 2px;">
                      <div class="card-header" style="padding: 4px;">
                        <span class="" style="font-size: 12px;">Transfer  
                          @if($transferthistenant!="")
                           <b>{{ $transferthistenant->Fname }} {{ $transferthistenant->Oname }}</b>
                          @endif
                          to:
                          @if($thishouse!="")
                      ...
                      <div class="card-body">
                        <div class="form-group">
                          <select class="form-control select2" name="alltenants" onchange="location=this.value;" style="width: 100%;">
                            <option value="">Search Tenant Transfering</option>
                            @forelse($tenantsinfo as $transfer)
                            @if($transferthistenant!="")
                              @if($transferthistenant->id==$transfer->id)
                                <option value="/properties/Transfer/Tenant/{{$thistenant->id}}/{{$thishouse->id}}/{{$transfer->id}}" selected>{{ $loop->index+1 }}. {{ $transfer->Fname }} {{ $transfer->Oname }} ({{ $transfer->Status }}) </option>
                              @else
                                <option value="/properties/Transfer/Tenant/{{$thistenant->id}}/{{$thishouse->id}}/{{$transfer->id}}">{{ $loop->index+1 }}. {{ $transfer->Fname }} {{ $transfer->Oname }} ({{ $transfer->Status }}) </option>
                              @endif
                            @else
                              <option value="/properties/Transfer/Tenant/{{$thistenant->id}}/{{$thishouse->id}}/{{$transfer->id}}">{{ $loop->index+1 }}. {{ $transfer->Fname }} {{ $transfer->Oname }} ({{ $transfer->Status }}) </option>
                            @endif
                            @empty
                              <option>No Vacant Tenant</option>
                            @endforelse
                          </select>
                        </div>

                        @if($transferthistenant=="")
                            <div class="text-danger"> No Tenant Selected</div>
                        @else
                        <div class="card-body row">
                            ...
                            <div class="col-sm-6 bg-info">
                              <dl class="row">
                                ...
                                <dd class="col-sm-8">{{ $transferthistenant->Fname }} {{ $transferthistenant->Oname }}</dd>
                                <dt class="col-sm-4">Gender</dt>
                                <dd class="col-sm-8">{{ $transferthistenant->Gender }}</dd>
                                <dt class="col-sm-4">Idno</dt>
                                <dd class="col-sm-8">{{ $transferthistenant->IDno }}</dd>
                                <dt class="col-sm-4">Phone</dt>
                                <dd class="col-sm-8">{{ $transferthistenant->Phone }} </dd>
                                <dt class="col-sm-4">Status</dt>
                                <dd class="col-sm-8">{{ $transferthistenant->Status }} </dd>
                                
                              </dl>
                            </div>
                            <div class="col-sm-6">
                              @if($thistenant->Status=="Assigned" || $thistenant->Status=="Reassigned" || $thistenant->Status=="Transferred")
                                <form role="form" class="form-horizontal" method="POST" action="/properties/savetenant/transfer/tenant">
                                    <div class="row">
                                    @csrf
                                    <div class="col-sm-12">
                                        <div class="card card-primary card-outline bg-info" style="text-align: center;">
                                          <div class="card-body">
                                            ...
                                            <input type="hidden" name="transferid" value="{{ $transferthistenant->id }}">
                                            ...
                                            <div class="form-group row">
                                                <label for="DateAssigned" class="col-md-4 col-form-label text-md-right">{{ __('Transfer') }}</label>
                                            ...
                                            <div class="form-group row">
                                                <div class="col-sm-12">
                                                    <button  class="btn btn-success btn-small btn-block" name="submitplotbtn" id="submitplotbtn"  type="submit" >Transfer {{ $transferthistenant->Fname }} {{ $transferthistenant->Oname }} to {{ $thishouse->Housename }}</button>
                                                </div>
                                            </div>
